...
def test_live_demo_approve_path():
    ...
    _timed("create", lambda: creator.create_task(
        args=["demo-paint-02", "Repaint the site office wall",
              "Wall painted clean light blue, neat white trim", 10, _sha(before)]
    ).transact())

    tx = _timed("submit-worker", lambda: worker.submit_proof(
        args=["demo-paint-02", _sha(after), _sha(before)]).transact())
    assert tx_execution_succeeded(tx)

    tx = _timed("confirm", lambda: creator.confirm_evidence(args=["demo-paint-02"]).transact())
    assert tx_execution_succeeded(tx)

    tx = _timed("resolve-AI", lambda: creator.resolve_task(
        args=["demo-paint-02", before, after]).transact())
    assert tx_execution_succeeded(tx)
    _consensus_summary(tx, "resolve approve")
    task = creator.get_task(args=["demo-paint-02"]).call()
    print("VERDICT:", task["verdict"])
    assert task["status"] == "APPROVED"
    assert task["worker"] != task["creator"]

    tx = _timed("claim-worker", lambda: worker.claim_reward(args=["demo-paint-02"]).transact())
    assert tx_execution_succeeded(tx)
    assert creator.get_task(args=["demo-paint-02"]).call()["status"] == "PAID"
    print("demo-paint-02 settled PAID")
...
def test_live_demo_reject_appeal_path():
    ...
    creator.create_task(
        args=["demo-roof-02", "Repaint the storage room wall",
              "Clean light blue paint, neat white trim", 7, ""]).transact()
    worker.submit_proof(args=["demo-roof-02", _sha(unrelated), _sha(unrelated)]).transact()
    creator.confirm_evidence(args=["demo-roof-02"]).transact()

    tx = _timed("resolve-AI-reject", lambda: creator.resolve_task(
        args=["demo-roof-02", unrelated, unrelated]).transact())
    assert tx_execution_succeeded(tx)
    _consensus_summary(tx, "resolve reject")
    task = creator.get_task(args=["demo-roof-02"]).call()
    print("VERDICT:", task["verdict"])
    assert task["status"] == "REJECTED"

    tx = _timed("appeal-worker", lambda: worker.appeal(
        args=["demo-roof-02", _sha(after), _sha(before)]).transact())
    assert tx_execution_succeeded(tx)
    creator.confirm_evidence(args=["demo-roof-02"]).transact()
    tx = _timed("resolve-AI-recovery", lambda: creator.resolve_task(
        args=["demo-roof-02", before, after]).transact())
    assert tx_execution_succeeded(tx)
    task = creator.get_task(args=["demo-roof-02"]).call()
    print("RECOVERED:", json.dumps(task["evidence_history"])[:400])
    assert task["status"] == "APPROVED"
    worker.claim_reward(args=["demo-roof-02"]).transact()
    assert creator.get_task(args=["demo-roof-02"]).call()["status"] == "PAID"
    print("demo-roof-02 settled PAID after appeal")
